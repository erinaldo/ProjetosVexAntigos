...
namespace Sistecno.Web.Fatura
{
    public partial class Fatura : System.Web.UI.Page
    {
        ...
        private void Carregar()
        {
            ...
            if (Request.QueryString["idtitulo"].ToString().Length < 11)
            {
                ds = Sistran.Library.GetDataTables.RetornarDataSetWS("EXEC RetornaDadosFatura " + Request.QueryString["idtitulo"], Session["cnx"].ToString());
                Session["idtitulo"] = Request.QueryString["idtitulo"];

            }
            else
            {
                ds = Sistran.Library.GetDataTables.RetornarDataSetWS("EXEC RetornaDadosFatura_guid '" + Request.QueryString["idtitulo"] +"'", Session["cnx"].ToString());

                if (ds.Tables[0].Rows.Count == 0)
                    return;

                Session["idtitulo"] =ds.Tables[0].Rows[0]["IdTitulo"].ToString();
