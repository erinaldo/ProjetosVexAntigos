    ...
    public partial class frmCriarZIP : System.Web.UI.Page
    {
        ...
        private void criarArquivosXML()
        {
            try
            {
                ...
                if (File.Exists(Server.MapPath("ZipsGerados") + "\\" + Session["idtitulo"] + ".zip"))
                {
                    ...
                    HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment;filename=" + Session["idtitulo"] + ".zip");

                    FileStream inStr = null;
                    byte[] buffer = new byte[30000];
                    long byteCount;

                    inStr = File.OpenRead(Server.MapPath("ZipsGerados") + "\\" + Session["idtitulo"] + ".zip");
            ...
            {
                    if (ds.Rows[i]["ULTIMOARQUIVOXML"].ToString().Trim() == "")
                        continue;
