...
namespace Sistecno.Web.Fatura
{
    public partial class frmGerarDocCob : System.Web.UI.Page
    {
        ...
        private void GerarOcoren()
        {
            ...
            //strsql += " left join DocumentoEletronico del on del.IdDocumento = doc.IdDocumento ";
            strsql += " WHERE ";
            strsql += " T.IDTITULO=" + Session["idtitulo"].ToString(); ;
            strsql += " AND DOC.TIPODEDOCUMENTO ='CONHECIMENTO'  and Nf.Serie is not null";
        ...
        private void VerificarNumeroXml(string strsql)
        {
            DataTable ds = Sistran.Library.GetDataTables.RetornarDataSetWS(strsql, Session["cnx"].ToString()).Tables[0];

            for (int i = 0; i < ds.Rows.Count; i++)
            {
                if (ds.Rows[i]["UltimoArquivoXML"].ToString() != "")
                {
                    XmlDocument d = new XmlDocument();
                    d.LoadXml(ds.Rows[i]["UltimoArquivoXML"].ToString());
                    d.Save(Server.MapPath("XmlGerados") + "\\" + ds.Rows[i]["UltimoArquivoXML"].ToString() + ".xml");

                    DataSet x = new DataSet();
                    x.ReadXml(Server.MapPath("XmlGerados") + "\\"+ ds.Rows[i][0].ToString() + ".xml");


                    if (x.Tables["ide"].Rows[0]["nCT"].ToString() != ds.Rows[i]["DNUMERO"].ToString())
                    {
                        string Up =  "Update Documento set Numero ='" + x.Tables["ide"].Rows[0]["nCT"].ToString() + "', DocumentoDoCliente4='" + x.Tables["infCte"].Rows[0][1].ToString().Replace("CTe", "") + "' where IdDocumento =" + ds.Rows[i]["IdDocumento"].ToString() + "; ";                        
                        Up  += "Update DocumentoEletronico set IdNota='" + x.Tables["infCte"].Rows[0][1].ToString().Replace("CTe", "") + "' where IdDocumento =" + ds.Rows[i]["IdDocumento"].ToString() + "; ";
                        Sistran.Library.GetDataTables.ExecutarComandoSql(strsql, Session["cnx"].ToString());
                    }
                }
            }
