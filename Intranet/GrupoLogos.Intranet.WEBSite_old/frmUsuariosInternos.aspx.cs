...
public partial class frmUsuariosInternos : System.Web.UI.Page
{
...
//           SistranBLL.Usuario.LogBDBLL.GravarLog(ILusuario[0].UsuarioId, ILusuario[0].Login, "ACESSOU " + Server.UrlDecode(Request.QueryString["opc"].ToString().ToUpper()), System.IO.Path.GetFileName(HttpContext.Current.Request.FilePath));
            //CarregarGrid();
           // btnNovo.Attributes.Add("OnClick", "javascript:window.open('frmUsuariosDetalheInterno.aspx?id=novo&ic=0&opc=" + Server.UrlEncode(Request.QueryString["opc"].ToString()).ToUpper() + "');");
    ...
    {
        DataTable dt = new SistranBLL.Usuario().ListarIntranet(txtLogin.Text.Trim(), txtNome.Text.Trim(), txtCPF.Text.Trim());
