...
public partial class ResumoPorFilial : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            clientesSelecionados = Sistran.Library.FuncoesUteis.retornarClientesResumoFilial();
            ...
            if (!IsPostBack)
            {
                ...
                if (clientesSelecionados != "")
                    strsql += " AND (NF.IDREMETENTE IN (" + clientesSelecionados + ")  OR NF.IDDESTINATARIO IN (" + clientesSelecionados + ")  OR NF.IDCLIENTE IN (" + clientesSelecionados + "))   ";
    ...
    protected void MontarTable(bool DesprezarNaoEntregues)
    {
        try
        {
            ...
            DataTable dt = NotasFiscais.ListarResumoPorFilial(Convert.ToDateTime(txtI.Text), Convert.ToDateTime(txtF.Text), clientesSelecionados, Session["Conn"].ToString(), en, series);
            dtRegra = (DataTable)Session["dtRegra"];
