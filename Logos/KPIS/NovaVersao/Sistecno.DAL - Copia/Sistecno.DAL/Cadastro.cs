    ...
    public class Cadastro
    {
        ...
        public int ProcurarIdImagemSite(int idCadastro, string cnx)
        {
            try
            {
                ...
                var oC = context.CadastroImagems.FirstOrDefault(i => i.IDCadastro == idCadastro && i.TipoImagem == "SITE");
        ...
        public DAL.Models.Cadastro RetornarTabela(int idCdastro, string cnx)
        {
            ...
            context.Database.Connection.ConnectionString = cnx;
            context.Database.Connection.Open();
            var oC = context.Cadastroes.FirstOrDefault(i => i.IDCadastro == idCdastro);
            context.Database.Connection.Close();
            return oC;
        }
        ...
        public DAL.Models.Cadastro RetornarByCnpj(string cnpj, string cnx)
        {
            ...
            context.Database.Connection.ConnectionString = cnx;
            context.Database.Connection.Open();
            var oC = context.Cadastroes.FirstOrDefault(i => i.CnpjCpf == cnpj);
            context.Database.Connection.Close();
            return oC;
        }
        ...
        public  void Alterar(DAL.Models.Cadastro cadastro, DataTable contatos, byte[] LogoTipo, string cnx)
        {
            ...
            try
            {
                ...
                var oC = context.Cadastroes.First(i => i.IDCadastro == cadastro.IDCadastro);
                ...
                for (int i = 0; i < contatos.Rows.Count; i++)
                {
                    if (contatos.Rows[i]["IDCADASTROCONTATOENDERECO"].ToString() == "0" && contatos.Rows[i]["STATUS"].ToString() == "SIM")//NOVO
                    {
                        ...
                        context.CadastroContatoEnderecoes.Add(cce);
                    }
                    else if (contatos.Rows[i]["STATUS"].ToString() == "SIM")//altera
                    {
                        int cod = int.Parse(contatos.Rows[i]["IDCADASTROCONTATOENDERECO"].ToString());
                        var occ = context.CadastroContatoEnderecoes.First(ii => ii.IDCadastroContatoEndereco == cod);
                    ...
                    else if (contatos.Rows[i]["IDCADASTROCONTATOENDERECO"].ToString() != "0" && contatos.Rows[i]["STATUS"].ToString() == "NAO")//EXCLUI
                    {
                        int cod = int.Parse(contatos.Rows[i]["IDCADASTROCONTATOENDERECO"].ToString());
                        var occ = context.CadastroContatoEnderecoes.First(ii => ii.IDCadastroContatoEndereco == cod);
                        context.CadastroContatoEnderecoes.Remove(occ);
                    }
                    context.SaveChanges();
                }
        ...
        public void GravarCadastro(DAL.Models.Cadastro cad, DataTable contatos, string cnx, DbTransaction trans, SistecnoContext context)
        {
            try
            {
        ...
        public class Motorista : Cadastro
        {
            ...
            /// Grava o Motorista
            /// </summary>
            /// <param name="cad">Dados do Cadastro de Motoristas</param>
            ...
            public void AlterarMotorista(DAL.Models.Cadastro cadastro, DAL.Models.CadastroComplemento cadastroComplemento, DataTable contatos, DAL.Models.Motorista mot, string cnx)
            {
                ...
                try
                {
                    ...
                    #region Cadastro
                    var oC = context.Cadastroes.FirstOrDefault(i => i.IDCadastro == cadastro.IDCadastro);
                    if (oC != null)
                    {
                    ...
                    var oCC = context.CadastroComplementoes.FirstOrDefault(i => i.IDCadastro == cadastro.IDCadastro);
                    ...
                    #region Motorista
                    var oM = context.Motoristas.FirstOrDefault(i => i.IDMotorista == mot.IDMotorista);
                    if (oM != null)
                    {
            ...
            public DAL.Models.Motorista RetornarTable(int idCadastro, string cnx)
            {
                try
                {
                    ...
                    context.Database.Connection.ConnectionString = cnx;
                    context.Database.Connection.Open();
                    var oC = context.Motoristas.FirstOrDefault(i => i.IDMotorista == idCadastro);
                    context.Database.Connection.Close();
                    return oC;
                }
                catch (Exception)
        ...
        public class CadadastroComplemento
        {
            public DAL.Models.CadastroComplemento RetornarTabela(int idCadastro, string cnx)
            {
                ...
                context.Database.Connection.ConnectionString = cnx;
                context.Database.Connection.Open();
                var o = context.CadastroComplementoes.First(i => i.IDCadastro == idCadastro);
                context.Database.Connection.Close();
                return o;
            }
        }
        ...
        public class Proprietario
        {
            public void MotoristaToProprietario(int idMotorista, bool p, string cnx)
            {
                if (idMotorista > 0)
                {
                    ...
                    context.Database.Connection.ConnectionString = cnx;
                    context.Database.Connection.Open();
                    var oC = context.Proprietarios.FirstOrDefault(i => i.IDProprietario == idMotorista);
                    ...
                    if (p && oC == null)
                    {
                        ...
                        context.Proprietarios.Add(pr);
                    }
                    else
                    {
                        context.Proprietarios.Remove(oC);
                    }
                    context.SaveChanges();
                    context.Database.Connection.Close();
                }
            ...
            public int Inserir(int idCadastro, string cnx)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        var oC = context.Proprietarios.FirstOrDefault(i => i.IDProprietario == idCadastro);
                        if (oC == null)
                            existe = false;
                        else
                            existe = true;
