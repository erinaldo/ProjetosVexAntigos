    ...
    public class Veiculo
    {
        ...
        public int GravarVeiculo(DAL.Models.Veiculo veic, string cnx)
        {
            ...
            try
            {
                if (veic.IDVeiculo == 0)
                {
                    ...
                    context.Veiculo.Add(veic);
                    context.SaveChanges();
                }
                else
                {
                    var o = context.Veiculo.First(i => i.IDVeiculo == veic.IDVeiculo);
        ...
        public DAL.Models.Veiculo RetornarAllFields(int idVeiculo, string cnx)
        {
            try
            {
                ...
                context.Database.Connection.ConnectionString = cnx;
                context.Database.Connection.Open();
                var oC = context.Veiculo.FirstOrDefault(i => i.IDVeiculo == idVeiculo);
                context.Database.Connection.Close();
                return oC;
            }
            catch (Exception)
        ...
        public DAL.Models.Veiculo RetornarbyPlaca(string placa, string cnx)
        {
            try
            {
                ...
                context.Database.Connection.ConnectionString = cnx;
                context.Database.Connection.Open();
                var oC = context.Veiculo.FirstOrDefault(i => i.Placa == placa);
                context.Database.Connection.Close();
                return oC;
            }
            catch (Exception)
