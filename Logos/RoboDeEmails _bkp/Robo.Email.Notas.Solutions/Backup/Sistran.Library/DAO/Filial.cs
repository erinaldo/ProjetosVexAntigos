...
namespace SistranDAO
{
    public  class Filial
    {
        ...
        {
            string strsql = "SELECT CAST(CF.IDFILIAL AS NVARCHAR(20))  VALOR , NOME + ' Unid.:' +  CAST(ISNULL(UNIDADE, '') AS NVARCHAR(20)) NOME FROM FILIAL INNER JOIN CLIENTEFILIAL CF ON CF.IDFILIAL = FILIAL.IDFILIAL WHERE IDCLIENTE in (" + Sistran.Library.FuncoesUteis.retornarClientes() + ") ORDER BY 2";
        ...
        {
            string strsql = "SELECT CAST(IDFILIAL AS NVARCHAR(20))  VALOR , NOME FROM FILIAL ORDER BY 2";
            return Sistran.Library.GetDataTables.RetornarDataTable(strsql.ToString(), Conn); //return SistranDb.ExecuteDataSet(CommandType.Text, strsql).Tables[0];
        }
        ...
        public DataTable ListarDisponiveisByIDMotorista(string Conn, int idMotorista)
        {
