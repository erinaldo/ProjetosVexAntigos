...
namespace SistranDAO
{
    public class Importacao
    {
        ...
        {
            string ID = Sistran.Library.GetDataTables.RetornarIdTabela("ESTOQUEMOV");
        ...
        {
            string ID = Sistran.Library.GetDataTables.RetornarIdTabela("DOCUMENTORELACIONADO");
        ...
        {
            string strsql = " SELECT ISNULL(IDDOCUMENTORELACIONADO,0) IDDOCUMENTORELACIONADO FROM DOCUMENTORELACIONADO WHERE IDDOCUMENTOPAI=" + CodPedido + "  AND IDDOCUMENTOFILHO = " + CodNotaFiscal;
            return Sistran.Library.GetDataTables.ExecutarRetornoID(strsql, "");            
        }

        public DataTable ConsultarDadosPed(string IDPRODUTOEMBALAGEM, string NumeroPedido)
        {
            string strsql = " SELECT DI.IDCLIENTEDIVISAO, QUANTIDADE, VALORUNITARIO, DC.IDDOCUMENTO   FROM DOCUMENTO DC INNER JOIN DOCUMENTOITEM DI ON DC.IDDOCUMENTO = DI.IDDOCUMENTO WHERE NUMERO = " + NumeroPedido + " AND DI.IDPRODUTOEMBALAGEM =" + IDPRODUTOEMBALAGEM;
        ...
        public int AbrirDT(int IdFilial, int Numero, int IdVeiculo, int IdMotorista, int IDDTTipo)
        {
            ...
            string strsql = "SELECT NUMERO FROM DT WHERE IDFILIAL=" + IdFilial.ToString() + " AND NUMERO=" + Numero.ToString();
