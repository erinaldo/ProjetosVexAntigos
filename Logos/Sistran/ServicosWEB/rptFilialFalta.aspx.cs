...
{
    public partial class rptFilialFalta : System.Web.UI.Page
    {
        string cnx = "";
        ...
        private void CarregarGrid()
        {
            ...
            string sql = " SELECT NOMEREGIAO FILIAL, COUNT(DISTINCT RRV.IDREPOSICAOROGEITEM ) FALTAS/*, cast(SUM(ISNULL(RR.VALOR,0)) as decimal(10,2)) [VALOR FALTAS]*/ ";
            sql += " FROM REPOSICAOROGE RR ";
            sql += " INNER JOIN REPOSICAOROGEITEM RRV ON RRV.IDREPOSICAOROGE = RR.IDREPOSICAOROGE ";
            sql += " WHERE STATUS ='FINALIZADO' ";

            if (txtDataI.Text != "" && txtDataF.Text != "")
                sql += " AND CONVERT(DATETIME, RR.DATAENVIOROGE, 103)  BETWEEN  CONVERT(DATETIME, '" + txtDataI.Text + " 00:00:00', 103)  AND CONVERT(DATETIME, '" + txtDataF.Text + " 23:59:59', 103)";

            if (cboStatus.SelectedIndex > 0)
                sql += " AND CODIGOREGIAO =" + cboStatus.SelectedValue;

            sql += " AND (RRV.DATACONFERIDO IS NULL OR RRV.QUANTIDADENOTA > ISNULL(RRV.QUANTIDADELIDO, 0))";
            sql += " GROUP BY RR.NOMEREGIAO";
            sql += " ORDER BY 2 DESC";



            //string sql = "SELECT NOMEREGIAO FILIAL, COUNT( DISTINCT RRV.IDREPOSICAOROGEVOLUME) VOLUMES ";
            //sql += " FROM REPOSICAOROGE RR ";
            //sql += " INNER JOIN REPOSICAOROGEVOLUME RRV ON RRV.IDRESPOSICAOROGE = RR.IDREPOSICAOROGE ";
            //sql += " WHERE 0=0 ";
            ...
            lblQuantidadeDocumentos.Text = ((DataTable)GridView1.DataSource).Compute("Sum(Faltas)", "")+ " Itens";
