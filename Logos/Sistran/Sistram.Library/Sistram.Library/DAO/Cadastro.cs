...
namespace SistranDAO
{
    public class Cadastro
    {
        public DataTable Read(string condicoes)
        {
            ...
            s.Append(" SELECT IDCADASTRO,CNPJCPF,INSCRICAORG,RAZAOSOCIALNOME,FANTASIAAPELIDO,ENDERECO,NUMERO,COMPLEMENTO,IDCIDADE,IDBAIRRO,CEP,CNPJCPFERRADO,INSCRICAOERRADA,DATADECADASTRO,CEPVALIDO,ANIVERSARIO,SUFRAMA,SUFRAMAVALIDADE,ORGAOEMISSOR,TIPODECADASTRO,SITUACAOFISCAL FROM CADASTRO  WITH (NOLOCK) WHERE " + condicoes);
        ...
        public int TransacaoInserirCadastroMotorista(string Conn, SistranMODEL.Cadastro oCad, SistranMODEL.Cadastro.Motorista oMot, SistranMODEL.Cadastro.Motorista.MotoristaHistorico oMotHst)
        {
            ...
            try
            {
                ...
                if (oMot.VencimentoPamcary != null)
                ...
                sql += " RecolheINSS, ";
sql += " RecolheIRRF, ";
sql += " RecolheSESTSENAT, ";
sql += " LocalEmissaoRG, senha ";
                sql += " ) ";
                sql += " VALUES ";
                sql += " ( ";
                sql += idCadastro.ToString() + " , 'CAPTACAO', ";
                ...
                sql += " '"+oMot.RecolheINSS+"', ";
                sql += " '"+oMot.RecolheIRRF+"', ";
                sql += " '"+oMot.RecolheSESTSENAT+"', ";
                sql += " '" + oMot.LocalEmissaoRG +"', ";
                ...
                if (oMot.VencimentoPamcary != null)
                {
                    DbParameter VencimentoPamcary = DbProviderFactories.GetFactory("System.Data.SqlClient").CreateParameter();
                    VencimentoPamcary.ParameterName = "@VencimentoPamcary";
                    VencimentoPamcary.Value = oMot.VencimentoPamcary;
                    VencimentoPamcary.DbType = DbType.DateTime;
                    cd.Parameters.Add(VencimentoPamcary);
                }
        ...
        public int TransacaoInserirCadastroProprietario(string Conn, SistranMODEL.Cadastro oCad, SistranMODEL.Cadastro.Proprietario oProp)
        {
            ...
            try
            {
                ...
                string sql = "Select count(IDCadastro) from cadastro where cnpjCPF='" + oCad.CnpjCpf + "'";
                cd.CommandText = sql;
                cd.CommandType = CommandType.Text;
                cd.Transaction = oTrans;
                ID = cd.ExecuteScalar().ToString();

                if (ID == "0")
                {
                    ID = Sistran.Library.GetDataTables.RetornarIdTabela("CADASTRO");
                ...
                {
                    sql = "Select count(IDCadastro) from cadastro where cnpjCPF='" + oCad.CnpjCpf + "'";
                    cd.CommandText = sql;
                    cd.CommandType = CommandType.Text;
                    cd.Transaction = oTrans;
                    ID = cd.ExecuteScalar().ToString();
                ...
                sql = "INSERT INTO PROPRIETARIO (IDPROPRIETARIO) VALUES (" + ID + ") ";
                cd.CommandText = sql;
                cd.Transaction = oTrans;
                ...
                GravarImagens(oTrans, cd, ID.ToString());
        ...
        public class CadastroReferencia
        {
            ...
            {
                string STR = "SELECT * FROM CADASTROREFERENCIA WHERE IDCADASTRO = "+idCadastro+" AND TIPODEREFERENCIA IN('COMERCIAIS','PESSOAIS')";
                return Sistran.Library.GetDataTables.RetornarDataTable(STR);
            }
        }
        ...
        public class Motorista
        {
            ...
            public void alterar(SistranMODEL.Cadastro.Motorista oMot, int IdCadastro)
            {
                ...
                else
                {
                    ...
                    sql += " senha='" + oMot.Senha + "', ";
                    sql += " RecolheINSS='"+oMot.RecolheINSS+"', ";
                    sql += " RecolheIRRF='"+oMot.RecolheIRRF+"', ";
                    sql += " RecolheSESTSENAT='"+oMot.RecolheSESTSENAT+"', ";
                    sql += " LocalEmissaoRG='" + oMot.LocalEmissaoRG +"', ";
                    ...
                    if (oMot.VencimentoPamcary.ToString() == "")
                    {
                        sql += " VencimentoPamcary=CONVERT(DATETIME,'" + oMot.VencimentoPamcary + "',103),";
                    }

                    if (oMot.VencimentoBrasilrisk.ToString() == "")
                    {
                        sql += " VencimentoBrasilrisk=CONVERT(DATETIME,'" + oMot.VencimentoPamcary + "',103),";
                    }

                    if (oMot.VencimentoBuonny.ToString() == "")
                    {
                        sql += " VencimentoBuonny=CONVERT(DATETIME,'" + oMot.VencimentoPamcary + "',103),";
                    }
                    ...
                    try
                    {
                        for (int i = 0; i < ListDeFiliais.Items.Count; i++)
                        {
                           // string ID = Sistran.Library.GetDataTables.RetornarIdTabela("MOTORISTAFILIAL");
                            ...
                            {
                                string ids = Sistran.Library.GetDataTables.RetornarIdTabela("MOTORISTAFILIAL");
        ...
        {
            public DataTable Pesquisar(string nome, string cpf, int? IdProprietario, int? idCadastro)
            {
                string strsql = "";

                if (IdProprietario == null)
                {
                    strsql = " SELECT * FROM CADASTRO C ";
                    strsql += " LEFT JOIN CADASTROCOMPLEMENTO CC ON CC.IDCADASTRO = C.IDCADASTRO ";
                    strsql += " WHERE  ";
                    strsql += " C.RAZAOSOCIALNOME LIKE '" + nome + "%' ";
                    strsql += " AND C.CNPJCPF LIKE '" + cpf + "%'";

                    if (idCadastro != null)
                        strsql += " AND C.IDCADASTRO = " + idCadastro;

                    strsql += " ORDER BY C.RAZAOSOCIALNOME";
                }
                else
                {
                    strsql = " SELECT * FROM PROPRIETARIO  P left JOIN CADASTRO C ON C.IDCADASTRO = P.IDPROPRIETARIO ";
                    strsql += " LEFT JOIN CADASTROCOMPLEMENTO CC ON CC.IDCADASTRO = C.IDCADASTRO ";
                    strsql += " WHERE  0=0";
