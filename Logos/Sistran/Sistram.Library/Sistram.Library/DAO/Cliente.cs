...
namespace SistranDAO
{
    public sealed class Cliente
    {
        ...
        public  SistranMODEL.Cliente RetornaCliente(int clienteIdCadastro, string Conn)
        {
        ...
        public static string RetornaDivisoesClientesUsuario(string idUsuario, string idCliente)
        {
            DataTable dtCompletas = DivisoesCompleta(idCliente);
            DataTable dtdivs = new DataTable();
            dtdivs.Columns.Add("IclienteDivisao");
            StringBuilder txt = new StringBuilder();
            txt.Append(" SELECT CliDiv.IDClienteDivisao ");
            txt.Append(" FROM   UsuarioCliente UsuCli ");
            txt.Append(" INNER JOIN UsuarioClienteDivisao UsuCliDiv on(UsuCliDiv.IDUsuarioCliente = UsuCli.IDUsuarioCliente)");
            ...
            foreach (DataRow item in t.Rows)
            {
                dtdivs.Rows.Add(item["IDClienteDivisao"].ToString());                
                CarregarFilhos(item["IDClienteDivisao"].ToString(), dtCompletas, dtdivs);
            }

            for (int i = 0; i < dtdivs.Rows.Count; i++)
                 m+= dtdivs.Rows[0][0].ToString() + ",";
        ...
        private static void CarregarFilhos(string p, DataTable dtCompletas, DataTable dtDivis)
        {            
            DataRow[] o = dtCompletas.Select("IdParente=" + p.ToString());

            for (int i = 0; i < o.Length; i++)
            {
                dtDivis.Rows.Add(o[i]["IDClienteDivisao"].ToString());
                if (dtCompletas.Select("idParente=" + o[i]["IDClienteDivisao"].ToString()).Length > 0) // aciona a recursividade
                    CarregarFilhos(o[i]["IDClienteDivisao"].ToString(), dtCompletas, dtDivis);
            }
        }
        ...
        {
            StringBuilder txt = new StringBuilder();
            txt.Append(" SELECT * FROM CLIENTEDIVISAO WHERE /*ATIVO = 'SIM' AND*/ IDCLIENTE='" + idCliente + "'");
        ...
        public DataTable RetornarClientesIntranet(string iniciais, string codigos)
        {
            ...
            s.Append(" AND DOC.DATADEEMISSAO>='2014-01-01' ");
            s.Append(" ORDER BY ISNULL(CAD.FANTASIAAPELIDO, CAD.RAZAOSOCIALNOME) ");
