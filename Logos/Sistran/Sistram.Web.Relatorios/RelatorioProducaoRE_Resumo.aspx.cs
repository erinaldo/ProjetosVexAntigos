...
public partial class RelatorioProducaoRE_Resumo : System.Web.UI.Page
{
    ...
    private void Gerar()
    {
        ...
        PhTotais.Controls.Clear();

        DataTable dtGeral = Sistran.Library.GetDataTables.RetornarDataSetWS("EXEC PRC_RE_MENSAL_LISTAR '" + DateTime.Parse(txtI.Text).ToString("yyyy-MM-dd") + "', '" + DateTime.Parse(txtF.Text).ToString("yyyy-MM-dd") + "'", Sistran.Library.Robo.Robo.RetornarStringBaseNovaLogos()).Tables[0];
