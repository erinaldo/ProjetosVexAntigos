...
public partial class Inicial : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            clientes = Sistran.Library.FuncoesUteis.retornarClientes();
            ...
            if (!IsPostBack)
            {
                ...
                Timer1.Enabled = true;
                Timer1.Interval = /*Convert.ToInt32(ConfigurationSettings.AppSettings["IntervaloRefresh"]) */ 5 * 60000;

                lblPeriodo.Text = inicio.ToShortDateString() + " à " + DateTime.Parse(DateTime.Now.ToShortDateString()).ToShortDateString();
                lblqtdEmitidas.Text = NotasFiscais.RetornarTotalNotasFiscaisEmitidas(clientes, inicio.ToShortDateString(), DateTime.Now.ToShortDateString()).ToString();
    ...
    protected void Montar(bool exibir, DateTime? ini, DateTime? fim)
    {
        ...
        try
        {
            ...
            DataTable dtDataFilialSituacao = NotasFiscais.ListarDataFilialSituacao(clientes, Session["Conn"].ToString(), "AGUARDANDO EMBARQUE", ini, fim,
                chkSeries.Items[0].Selected, chkSeries.Items[2].Selected, chkSeries.Items[1].Selected, chkClientes.Items[0].Selected);
            ...
            if (dtDataFilialSituacao.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dsv.Rows.Count; i++)
                {
                    ...
                    phAguardandoEmbarqueDataFilial.Controls.Add(new LiteralControl(@"<td class='tdpCabecalho' align='right' nowrap=nowrap style='font-size:8pt;width:1%'><a href='InicialDetalhe.aspx?esp="+chkClientes.SelectedValue+"&idfilial=" + dsv.Rows[i]["IDFILIAL"].ToString() + "&data=&tipo=agemb&opc=&"+par+"' class='link' target='_blank' >" + NomeFilial + "</a>"));
                    phAguardandoEmbarqueDataFilial.Controls.Add(new LiteralControl(@"</td>"));
                ...
                for (int ii = 0; ii < intev; ii++)
                {
                    ...
                    if (ot.Length > 0)
                    {
                        string par = "os=" + chkSeries.Items[0].Selected + "&d=" + chkSeries.Items[1].Selected + "&r=" + chkSeries.Items[2].Selected; 
                        phAguardandoEmbarqueDataFilial.Controls.Add(new LiteralControl(@"<tr>"));
                        phAguardandoEmbarqueDataFilial.Controls.Add(new LiteralControl(@"<td class='tdpCenter' nowrap=nowrap align='center'><a href='InicialDetalhe.aspx?esp=" + chkClientes.SelectedValue + "&opc=Aguardando Embarque&idfilial=&data=" + dtInicio.ToShortDateString() + "&tipo=agemb&opc=&" + par + "' class='link' target='_blank'>" + dtInicio.ToShortDateString() + "</a>")); ;
                        phAguardandoEmbarqueDataFilial.Controls.Add(new LiteralControl(@"</td>"));
            ...
            if (dtDataFilialSituacao.Rows.Count > 0 && cboSituacao.Items.Count > 0)
            {
                ...
                for (int i = 0; i < dsv.Rows.Count; i++)
                {
                    ...
                    string NomeFilial = dsv.Rows[i]["NOME"].ToString().ToString().Replace("PROMOCIONAL", "PROM.").Replace("SAO JOSE DOS CAMPOS", "S. J. CAMPOS").Replace("SAO BERNARDO DO CAMPO", "S.B.C.").Replace("TABOAO", "EMBU");
                    phPorSituacao.Controls.Add(new LiteralControl(@"<td class='tdpCabecalho' align='right' nowrap=nowrap style='font-size:8pt;width:1%'><a href='InicialDetalhe.aspx?esp=" + chkClientes.SelectedValue + "&opc=" + cboSituacao.SelectedValue + "&idfilial=" + dsv.Rows[i]["IDFILIAL"].ToString() + "&data=&tipo=" + cboSituacao.SelectedValue + "&opc=&" + par + "' class='link' target='_blank' >" + NomeFilial + "</a>"));
                    phPorSituacao.Controls.Add(new LiteralControl(@"</td>"));
                ...
                for (int ii = 0; ii < intev; ii++)
                {
                    ...
                    if (ot.Length > 0)
                    {
                        ...
                        phPorSituacao.Controls.Add(new LiteralControl(@"<tr>"));
                        phPorSituacao.Controls.Add(new LiteralControl(@"<td class='tdpCenter' nowrap=nowrap align='center'><a href='InicialDetalhe.aspx?esp=" + chkClientes.SelectedValue + "&idfilial=&data=" + dtInicio.ToShortDateString() + "&tipo=" + cboSituacao.SelectedValue + "&opc=&" + par + "' class='link' target='_blank'>" + dtInicio.ToShortDateString() + "</a>")); ;
                        phPorSituacao.Controls.Add(new LiteralControl(@"</td>"));
