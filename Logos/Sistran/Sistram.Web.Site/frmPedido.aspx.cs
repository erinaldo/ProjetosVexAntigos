        ...
        try
        {
           ...
           // CarregarMenuDivisao();
            if (!IsPostBack)
            {
                ...
                {
                    if (Request.QueryString["Page"] != null && int.Parse(Request.QueryString["Page"])>0 )
                    {
                        carregerDataList((DataTable)Session["dtsTemp"]);
                        return;
                    }
                    List<SistranMODEL.Usuario> LUser = (Session["USUARIO"] as List<SistranMODEL.Usuario>);
                    DataTable dt = new SistranBLL.Produto().ListarProdutoAleatorio(Convert.ToInt32(Session["IDEmpresa"]));
                    lblMensagem.Text = "Selecione uma opção ao lado.";
                    ...
                    if (DataList1.Items.Count == 0)
                    {
                       // string[] x = Session["PrimeiraDivisao"].ToString().Split(new char[] { '|' });
                        ...
                        dt = new SistranBLL.Produto().ListarProdutoByDivisaoCliente(0, Convert.ToInt32(Session["IDEmpresa"]), true);
///                        dt = new SistranBLL.Produto().ListarProdutoByDivisaoCliente(Convert.ToInt32(x[0]), Convert.ToInt32(Session["IDEmpresa"]), true);
    ...
    {        
       ...
       // CollectionPager1.PageSize = dt.Rows.Count + 1;
    ...
    protected void DataList1_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            ...
            if (Image1 != null)
            {
                lblCodigo.Text = dv["IDPRODUTOCLIENTE"].ToString();
                ...
                lblDivisao.Text = dv["NOME"].ToString();

                if (lblDivisao.Text.Trim() == "")
                {
                    lblDivisao.Text = dv["Divisao"].ToString();

                }

                btnADDCart.CommandArgument = dv["IDPRODUTOCLIENTE"].ToString();
    ...
    protected void DataList1_ItemCommand(object source, DataListCommandEventArgs e)
    {
        if (e.CommandArgument.ToString() != "" && e.CommandName != "")
        {
            ...
            if(File.Exists(MapPath("imgReport") + "\\" +  e.CommandArgument.ToString() + ".jpg"))
                imgPop.ImageUrl = "imgReport/" + e.CommandArgument.ToString() + ".jpg";            


            DataTable dt = (DataTable)Session["dtsTemp"];
            DataRow[] ow = dt.Select("IDPRODUTOCLIENTE=" + e.CommandArgument.ToString() + " AND IDCLIENTEDIVISAO='" + e.CommandName + "'", "");
            
            if (ow.Length > 0)
            {
                lblCodigo.Text = ow[0]["IDPRODUTOcliente"].ToString();
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    lblMetro.Text = dtProd.Rows[0]["Rodoviario"] == DBNull.Value?"0": Convert.ToDecimal(dtProd.Rows[0]["Rodoviario"]).ToString("#0.000");
                }
            }
        }
    }
    ...
    private void FiltarPorNome()
    {
        ...
        lblMensagem.Text = "VOCÊ PESQUISOU POR NOME: " + txtBuscar.Text;
