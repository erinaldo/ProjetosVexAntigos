...
{
    public partial class CLW00018 : Form
    {
        public CLW00018()
        {
            InitializeComponent();
        ...
        {
            DialogResult result = MessageBox.Show("Tem Certeza que dejesa Zerar Esta UA, esta ação é INRREVERSIVEL", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
        ...
        private void Salvar()
        {
            try
            {
                if (txtUa.Text != "")
                {
                    new Classes.BLL.Estoque().SairUAInteira(dtUa.Rows[0]["IdUnidadeDeArmazenagem"].ToString(), lblIdProdutoCliente.Text, lblSaldoUa.Text, txtEndereco.Text, "5", dtUa.Rows[0]["IDPRODUTOEMBALAGEM"].ToString());
            ...
            {
                MessageBox.Show("Saldo de Estoque insuficiente, favor anotar o endereço e contactar o suporte");
            }
        }
        ...
        private void txtEndereco_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                ...
                if (e.KeyChar == (char)Keys.Return && txtEndereco.Text != "")
                {
                    ...
                    string sql = "SELECT UA.IDDepositoPlantaLocalizacao,UAL.Saldo, ua.IDUnidadeDeArmazenagem, ua.digito, pe.idprodutocliente FROM UnidadeDeArmazenagem UA ";
                    ...
                    sql += " inner join ProdutoEmbalagem pe on ua.IdProdutoEmbalagem = pe.IDProdutoEmbalagem ";

                    sql += " WHERE DPL.IDDepositoPlantaLocalizacao = " + txtEndereco.Text;
                    sql += " AND UAL.Saldo >0 ";


                    DataTable dtemp = Classes.BdExterno.RetornarDT(sql, VarGlobal.Conexao);
                    if (dtemp.Rows.Count == 0)
                    {
                        MessageBox.Show("Endereco Vazio");
                        btnConfirmar.Enabled = false;
                    ...
                    {
                        txtUa.Text = dtemp.Rows[0]["IDUnidadeDeArmazenagem"].ToString() + dtemp.Rows[0]["digito"].ToString();
                        lblIdProdutoCliente.Text = dtemp.Rows[0]["idprodutocliente"].ToString();
                        PesquisarUA();                        
            ...
            {
                MessageBox.Show("Problema ao encontrar as uas no endereco");
            }
        }
