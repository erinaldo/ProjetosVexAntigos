...
namespace RsMobile
{
    public class Bb
    {
        ...
        private string RetornarConexao()
        {
            string path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().GetName().CodeBase), "Transporte.sdf");
            return "Data Source=" + path;
        }

        private void excSql(List<string> comandos)
        {
        ...
        private DataTable RetornaDataTable(string sql)
        {
            SqlCeConnection cnn = new SqlCeConnection(RetornarConexao());
            SqlCeCommand cmd = new SqlCeCommand();
