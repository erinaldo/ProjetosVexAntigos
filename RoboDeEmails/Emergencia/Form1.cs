...
namespace Emergencia
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //DataTable dt = Sistran.Library.GetDataTables.RetornarDataTableWS("exec caio", cnx);
            ...
            //    try
            //    {
            //        byte[] imageBytes = (byte[])dt.Rows[i]["foto"];
            //        MemoryStream buf = new MemoryStream(imageBytes);
            //        Image image = Image.FromStream(buf, true);
            //        image.Save("D:\\imgCaio\\" + dt.Rows[i]["Numero"].ToString() + "_" + dt.Rows[i]["IdOcorrenciaComprovei"].ToString() + ".jpg");
            //    }
            //    catch (Exception)
            //    {
            //        continue;
        ...
        private void AcertarNotasNoStistranetDoComprovei()
        {
            ...
            try
            {
                ...
                DataTable dt = Sistran.Library.GetDataTables.RetornarDataTableWin(sql, "Data Source=192.168.10.10;Initial Catalog=VEX20200631;User ID=sa;Password=WERasd27;");
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (ret.Rows.Count == 0)
                    {
                            ...
                            if (aux.Rows.Count > 0)
                            {
                                ...
                                if (dataDaOcorrencia.ToString("dd/MM/yyyy HH:mm") == dataOcorrenciaJaExistente.ToString("dd/MM/yyyy HH:mm"))
                                {
                                    ...
                                    if (existeFoto.Rows.Count > 0)
                                    {
                                        ...
                                        DataTable ccc = Sistran.Library.GetDataTables.RetornarDataTableWin(strsql, "Data Source=192.168.10.10;Initial Catalog=VEX20200631;User ID=sa;Password=WERasd27;");
                                    ...
                                    //base arquivos
                                    Sistran.Library.GetDataTables.ExecutarRetornoIDWIN("Update RetornoComprovei set Processado=getdate() where IdRetornoComprovei=" + dt.Rows[i]["IdRetornoComprovei"].ToString() + "; select 1", "Data Source=192.168.10.10;Initial Catalog=VEX20200631;User ID=sa;Password=WERasd27;");
                                    continue;
                                }
                                ...
                                if (dataDaOcorrencia.ToString("dd/MM/yyyy") == dataOcorrenciaJaExistente.ToString("dd/MM/yyyy"))
                                {
                                    ...
                                    if (existeFoto.Rows.Count > 0)
                                    {
                                        //base arquivo
                                        strsql = "UPDATE DOCUMENTOOCORRENCIAARQUIVO SET ARQUIVO=(select foto from RETORNOCOMPROVEI  WITH (NOLOCK) where IdRetornoComprovei=" + dt.Rows[i]["IdRetornoComprovei"].ToString() + ") WHERE IDDOCUMENTOOCORRENCIA=" + aux.Rows[0]["IDDOCUMENTOOCORRENCIA"].ToString() + " ; select 1";
                                         Sistran.Library.GetDataTables.RetornarDataTableWin(strsql, "Data Source=192.168.10.10;Initial Catalog=VEX20200631;User ID=sa;Password=WERasd27;");
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            string id = Sistran.Library.GetDataTables.RetornarIdTabela("DOCUMENTOOCORRENCIAARQUIVO", Sistran.Library.Robo.Robo.RetornarStringBaseNovaLogos());
                                            strsql = "insert into DOCUMENTOOCORRENCIAARQUIVO values (" + id + ", " + aux.Rows[0]["IDDOCUMENTOOCORRENCIA"].ToString() + ", (select foto from RETORNOCOMPROVEI  WITH (NOLOCK) where IdRetornoComprovei=" + dt.Rows[i]["IdRetornoComprovei"].ToString() + ")) ; select 1";
                                            Sistran.Library.GetDataTables.RetornarDataTableWin(strsql, "Data Source=192.168.10.10;Initial Catalog=VEX20200631;User ID=sa;Password=WERasd27;");
                                        }
                                    }
                                    ...
                                    //base arquivo
                                    Sistran.Library.GetDataTables.ExecutarRetornoIDWIN("Update RetornoComprovei set Processado=getdate() where IdRetornoComprovei=" + dt.Rows[i]["IdRetornoComprovei"].ToString(), "Data Source=192.168.10.10;Initial Catalog=VEX20200631;User ID=sa;Password=WERasd27;");
                        ...
                        if (dt.Rows[i]["foto"].ToString() != "")
                        {
                            ...
                            Sistran.Library.GetDataTables.ExecutarRetornoIDWIN(strsql, "Data Source=192.168.10.10;Initial Catalog=VEX20200631;User ID=sa;Password=WERasd27;");
