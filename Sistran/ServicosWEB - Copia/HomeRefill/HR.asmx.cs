...
namespace ServicosWEB.HomeRefill
{
    ...
    public class HR : System.Web.Services.WebService
    {
        ...
        {
            SqlConnection cnx = new SqlConnection(ConfigurationSettings.AppSettings["cnxHomeRefill"].ToString());
        ...
        public DataTable Entradas(string Login, string Senha, string DataInicial, string DataFinal)
        {
            ...
            try
            {
                ...
                sql += "   INNER JOIN MOVIMENTACAOITEM MI  WITH (NOLOCK) ON MI.IDMOVIMENTACAO = M.IDMOVIMENTACAO ";
                sql += " INNER JOIN DOCUMENTO DOC  WITH (NOLOCK) ON DOC.IDDOCUMENTO = MI.IDDOCUMENTO ";
                sql += " INNER JOIN CADASTRO F  WITH (NOLOCK) ON F.IDCADASTRO = DOC.IDREMETENTE ";
                sql += " INNER JOIN UNIDADEDEARMAZENAGEMLOTE UAL  WITH (NOLOCK) ON UAL.IDUNIDADEDEARMAZENAGEMLOTE=MI.IDUNIDADEDEARMAZENAGEMLOTE ";
                sql += " INNER JOIN LOTE LT  WITH (NOLOCK) ON LT.IDLOTE = UAL.IDLOTE ";
                sql += " INNER JOIN MAPA MP  WITH (NOLOCK) ON MP.IDMAPA = MI.IDMAPA ";
                sql += " INNER JOIN PRODUTOEMBALAGEM PE  WITH (NOLOCK) ON PE.IDPRODUTOEMBALAGEM = MI.IDPRODUTOEMBALAGEM ";
                sql += " INNER JOIN PRODUTOCLIENTE PC  WITH (NOLOCK) ON PC.IDPRODUTOCLIENTE = PE.IDPRODUTOCLIENTE ";
                sql += " INNER JOIN PRODUTO P  WITH (NOLOCK) ON P.IDPRODUTO=PE.IDPRODUTO ";
                ...
                if (DataInicial == "" || DataFinal == "")
                    sql += " AND  (MI.OBS  NOT LIKE '%ENVIADO%'  OR MI.OBS IS NULL)  AND  M.DATADECADASTRO >= getDate()-30  ";
                else
                ...
                sql += " and doc.DataDeEntrada >= getdate()-90 and TipoDeDocumento='nota fiscal'";
        ...
        public DataTable SaldoAtual(string Login, string Senha)
        {
            ...
            string numCtr = random.Next().ToString();
            ...
            try
            {
                ...
                string s = "insert into LogSaldoAtual(erro) values ('" + numCtr + " INICIO - CONSULTA CORRETA')"; ;
                ...
                for (int i = 0; i < d.Rows.Count; i++)
                {
                    try
                    {
                        ...
                        if (float.Parse(d.Rows[i]["SALDOATUAL"].ToString()) < 0)
                ...
                try
                {
                    s = "insert into LogSaldoAtual(erro) values ('"+ numCtr +" FIM - CONSULTA CORRETA')";
                    Sistran.Library.GetDataTables.ExecutarSemRetornoWin(s, cnx);
                }
                catch (Exception)
                { }
            ...
            catch (Exception exx)
            {
                ...
                try
                {
                    string s = "insert into LogSaldoAtual(erro) values ('" + numCtr + " " + exx.Message.Replace("'", "").Replace("  ", " ") + "')";
                    Sistran.Library.GetDataTables.ExecutarSemRetornoWin(s, cnx);
        ...
        //    string obj = "";
            ...
            try
            {
                ...
                foreach (var p in pedido)
                {
                    ...
                    if (p.Longitude != "" && p.Latitude != "")
                    {
                        ...
                        Pe pe = new Pe(); //ProcurarEndereco.ProcurarPeloGoogle(p.Longitude, p.Latitude, cnx, p.Dest_ENTREGA_ENDERECO);
                       ...
                       // if (pe.CEPP == "SAO PAUL")
                        ...
                        if (pe.CEPP != null)
                        {
                            ...
                            if (p.Dest_CIDADE_NOME != null && p.Dest_CIDADE_NOME.Length > 1)
                                ne.IdCidade = ProcurarEndereco.RetornarCidade(p.Dest_CIDADE_NOME, (p.Dest_CEP != "" ? p.Dest_CEP.Replace("-", "") : pe.CEPP), cnx);
                            else
                        ...
                        else//Problema no cep
                        {
                            ne.CEP = "00000000";
                            ne.Complemento = p.Dest_ENTREGA_COMPLEMENTO.Replace("'", "");
                            ne.IdCidade = 0;
                            p.Dest_BAIRRO = "Nao encontrado";
                            ne.IdBairro = 0;
                        }
                ...
                cnx = ConfigurationSettings.AppSettings["cnxHomeRefill"].ToString();;
                ...
                for (int i = 0; i < pedido.Count; i++)
                {
                    ...
                    sql = " SELECT count(*)  FROM DOCUMENTO WHERE IDCLIENTE=150000 AND NUMERO ='" + pedido[i].NumeroDocumento + "' AND TIPODEDOCUMENTO='PEDIDO' AND SERIE = '" + pedido[i].Serie + "' AND ENTRADASAIDA='SAIDA' AND ATIVO='SIM' AND DATADECANCELAMENTO IS NULL ";
                    ...
                    if (tipo.ToUpper() == "'ENTRADA'")
                    {
                        ...
                        sql = " SELECT count(*)  FROM DOCUMENTO WHERE IDCLIENTE=150000 AND  IDREMETENTE=" + idc + " AND NUMERO ='" + pedido[i].NumeroDocumento + "' AND TIPODEDOCUMENTO='PEDIDO' AND SERIE = '" + pedido[i].Serie + "' AND ENTRADASAIDA='ENTRADA' AND ATIVO='SIM' AND DATADECANCELAMENTO IS NULL ";
                    ...
                    if (TipoDePedido == "EFETIVO" || tipo=="ENTRADA")
                        ...
                        ssqlFinal = ssqlFinal.Replace("@SERIE", "PED");

                    
                    ssqlFinal = ssqlFinal.Replace("@TIPODEDOCUMENTO", pedido[i].TipoDeDocumento.ToUpper());
            ...
            {               
                ...
                try
                {
                    Sistran.Library.EnviarEmails.EnviarEmail("moises@sistecno.com.br", "moises@sistecno.com.br", "Erro - Pedido HR ", "Ex:" + ex.Message + ssqlFinal, "mail.sistecno.com.br", "@oncetsis12122014", "HOMEREFILL");
                }
                catch (Exception ex2)
                { }
...
public static class ProcurarEndereco
{
    ...
    public static int RetornarCidadeIBGE(string CbIBGE, string CEP, string cnx)
    {
        

        string sql = "select * from Cidade where CodigoDoIBGE=" + CbIBGE;
        DataTable dt = Sistran.Library.GetDataTables.RetornarDataSetWS(sql, cnx).Tables[0];
