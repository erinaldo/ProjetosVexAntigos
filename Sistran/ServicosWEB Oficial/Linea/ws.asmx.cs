...
using static ProcurarEndereco3;

namespace ServicosWEB.Linea
{
    /// <summary>
    ...
    public class ws : System.Web.Services.WebService
    {
        ...
        private void CadastrarProdutosLinea(Recebimento Recebimento)
        {
            string cnx = Sistran.Library.Robo.Robo.RetornarStringBaseNovaLogos();
            ...
            for (int iped = 0; iped < Recebimento.Itens.Count; iped++)
            {
                ...
                string sql = "SELECT * FROM PRODUTOCLIENTE PC WHERE IDCLIENTE= 9999 AND CODIGO = '" + Recebimento.Itens[iped].Codigo.Trim() + "'";
                DataTable dProdCli = Sistran.Library.GetDataTables.RetornarDataTableWS(sql, cnx);
                ...
                if (dProdCli.Rows.Count == 0)
                {
                    ...
                    sqlGeral += "; INSERT INTO PRODUTOCLIENTE (IDProdutoCliente, IDCliente, IDUnidadeDeMedida,Codigo, Descricao,MetodoDeMovimentacao, DesmembraNaNF, SolicitarDataDeValidade,UnidadeDoFornecedor,Ativo, CodigoNCM, FatorUsoPosicaoPallet) ";
                    sqlGeral += " VALUES(" + IdprodCli + ", " + 9999 + ", 1,'" + Recebimento.Itens[iped].Codigo.ToUpper().Trim() + "', '" + Recebimento.Itens[iped].Descricao.Trim().ToUpper().Replace("'", "") + "','FEFO', 'NAO', 'SIM',1.00,'SIM', '', 1)";
        ...
        [WebMethod]
        [SoapLoggerExtensionAttribute(Filename = "C:\\temp\\Pedido\\NEEntradaLinea.log")]
        public string RemessaDeRecebimentoTeste(string Login, string Senha, Recebimento Recebimento, string CNPJdoCliente)
        {
            ...
            try
            {
                if ((Login != "Linea" && Senha != "@linea2020") && (Login != "321500" && Senha != "321500"))
                ...
                string sql = " SELECT count(*)  FROM DOCUMENTO WHERE IDCLIENTE=" + idDest + " and IDDestinatario=9999  AND  IDREMETENTE=" + idRem + " AND NUMERO ='" + Recebimento.NumeroNfe + "' AND TIPODEDOCUMENTO='NOTA FISCAL' AND SERIE = '" + Recebimento.Serie + "' AND ENTRADASAIDA='ENTRADA' AND ATIVO='SIM' AND DATADECANCELAMENTO IS NULL ";
                ...
                CadastrarProdutosLinea(Recebimento);
            ...
            {
                return "Não foi possível cadastrar o Recebimento. " + ex.Message;
            }
        }
        ...
        public string RemessaDeRecebimento(string Login, string Senha, Recebimento Recebimento, string CNPJdoCliente)
        {
            ...
            try
            {
                ...
                string idDocumento = Sistran.Library.GetDataTables.RetornarIdTabela("Documento", cnx);
                string sqlGeral = " Insert into Documento (DocumentodoCliente4, IDDocumento,IDFilial,IDFilialAtual,TipoDeServico,TipoDeDocumento,Serie,Numero,AnoMes,NumeroOriginal,IDCliente,IDRemetente,IDDestinatario,ClasseCFOP,Origem,EntradaSaida,DataDeEntrada,DataDeEmissao,Ativo,Endereco,EnderecoNumero,EnderecoComplemento,IDEnderecoBairro,IDEnderecoCidade,EnderecoCep,CodigoDoRecExpImpresso) ";
                sqlGeral += "values ('" + Recebimento.Chave + "'," + idDocumento + ",15,15,'NORMAL','NOTA FISCAL','" + Recebimento.Serie + "','" + Recebimento.NumeroNfe + "','" + DateTime.Parse(Recebimento.DataEmissaoNf.ToString()).ToString("yyyy/MM") + "','', " + idDest + "," + idRem + "," + idDest + ",5,'WS_ROBO','ENTRADA',GETDATE(),'" + Recebimento.DataEmissaoNf.ToString("yyyy-MM-dd") + "','NAO','" + dtDest.Rows[0]["Endereco"].ToString() + "','" + dtDest.Rows[0]["Numero"].ToString() + "','" + dtDest.Rows[0]["Complemento"].ToString() + "', '" + dtDest.Rows[0]["IdBairro"].ToString() + "','" + dtDest.Rows[0]["IdCidade"].ToString() + "','" + dtDest.Rows[0]["CEP"].ToString() + "','NAO') ";
        ...
        public RetornoStatusNfes RetornarStatusNfe(string usuario, string senha, string chave)
        {
            ...
            if ((usuario == "Linea" && senha == "@linea2020") || (usuario == "via" && senha == "@via321500"))
            {
        ...
        public string ReceberPedidosStatus(string Login, string Senha, List<Pedido> pedido, string prod_hom, string TipoDePedido, string CNPJdoCliente)
        {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    Sistran.Library.EnviarEmails.EnviarEmail("moises@sistecno.com.br", "moises@sistecno.com.br", "Erro - Pedido Linea HR ", "Ex:" + ex.Message + ssqlFinal, "mail.sistecno.com.br", "@oncetsis12122014", "HOMEREFILL");
                }
                catch (Exception ex2)
                { }
        ...
        public List<RetornoPedidoStatus> PedidosPorStatus(string Login, string Senha, string status)
        {
            ...
            try
            {
                ...
                string sql = "Select a.* from YandehStatusPedido a  " +
                    ...
                    "and   b.IDCliente = 9999   and EntradaSaida = 'SAIDA'   " +
                    "Order by Id";
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (ret.Status.ToUpper().Trim() == "LIBERADO PARA FATURAMENTO")
                    {
                        ...
                        if (dtq.Rows.Count == 0 || dtq.Rows[0][0].ToString() == "0")
                        {
                            try
                            {
                                Sistran.Library.GetDataTables.RetornarDataTableWS("delete from YandehStatusPedido where Id= " + dt.Rows[i]["Id"].ToString() + "; select 1", cnx);
                                //string s = " update MovimentacaoItem set GeradoYadeh = null where IDDocumento = "+ dt.Rows[i]["IdDocumento"].ToString() + "; Update DocumentoFilial set Situacao = ''  where IDDocumento = "+ dt.Rows[i]["IdDocumento"].ToString() + " ;Update DocumentoFilial set Situacao = 'LIBERADO PARA FATURAMENTO'  where IDDocumento = " + dt.Rows[i]["IdDocumento"].ToString();
                                //Sistran.Library.GetDataTables.ExecutarComandoSqlTrans(s, cnx);

                                //Sistran.Library.GetDataTables.RetornarDataTableWS("Update YandehStatusPedido set status='LIBERADO PARA FATURAMENTO.', Consumido=getdate() where Id= " + dt.Rows[i]["Id"].ToString() + "; select 1", cnx);


                                Sistran.Library.EnviarEmails.EnviarEmail("moises@sistecno.com.br", "moises@sistecno.com.br", "Pedido Corte Total Errado ", "Pedido: " + dt.Rows[i]["NRPedido"].ToString(), "mail.sistecno.com.br", "@oncetsis12122014", "Giro");
...
public static class ProcurarEndereco3
{
