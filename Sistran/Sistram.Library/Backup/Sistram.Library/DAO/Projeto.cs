    ...
    public sealed class Projeto
    {
                            ...
                            )
        {
            try
            {
                ...
                if (IDPROJETO == 0)
                {
                    ID = Sistran.Library.GetDataTables.RetornarIdTabela("PROJETO", HttpContext.Current.Session["ConnLogin"].ToString());
                    ...
                    strsql += " STATUS, ";
                    strsql += " Edi, ";
                    strsql += " DigitalizarComprovante, ";
                    strsql += " EmitirCTRC, ";
                    strsql += " EmitirNotaFiscalServico, ";
                    strsql += " FormaFaturamento, ";
                    strsql += " Vencimento, ";
                    strsql += " OrigemDaColeta, ";
                    strsql += " ValorPorColeta, ";
                    strsql += " PlanejamentoDeTransferencia, ";
                    strsql += " Observacao";
                ...
                else
                {
                    ...
                    strsql += " Edi='" + Edi + "', ";
                    strsql += " DigitalizarComprovante='" + DigitalizarComprovante + "', ";
                    strsql += " EmitirCTRC='" + EmitirCTRC + "', ";
                    strsql += " EmitirNotaFiscalServico='" + EmitirNotaFiscalServico + "', ";
                    strsql += " FormaFaturamento='" + FormaFaturamento + "', ";
                    strsql += " Vencimento='" + Vencimento + "', ";
                    strsql += " OrigemDaColeta='" + OrigemDaColeta + "', ";
                    strsql += " ValorPorColeta='" + ValorPorColeta + "', ";
                    strsql += " PlanejamentoDeTransferencia='" + PlanejamentoDeTransferencia + "', ";
                ...
                if (HttpContext.Current.Session["filialRecebimento"] != null)
                {
                    ListBox lstRecebidos = (ListBox)HttpContext.Current.Session["filialRecebimento"];
                    Sistran.Library.GetDataTables.ExecutarSemRetorno("DELETE FROM ProjetoFilial WHERE IDPROJETO=" + ID + " AND TIPO='RECEBIMENTO'", "");

                    for (int i = 0; i < lstRecebidos.Items.Count; i++)
                    {
                        strsql = "INSERT INTO ProjetoFilial VALUES (" + Sistran.Library.GetDataTables.RetornarIdTabela("PROJETOFILIAL") + ", " + ID + "," + lstRecebidos.Items[i].Value + ",'RECEBIMENTO')";
                        Sistran.Library.GetDataTables.ExecutarSemRetorno(strsql, "");
                    }
                }


                if (HttpContext.Current.Session["filialEntrega"] != null)
                {
                    ListBox lstEntrega = (ListBox)HttpContext.Current.Session["filialEntrega"];
                    Sistran.Library.GetDataTables.ExecutarSemRetorno("DELETE FROM ProjetoFilial WHERE IDPROJETO=" + ID+ " AND TIPO='ENTREGA'", "");

                    for (int i = 0; i < lstEntrega.Items.Count; i++)
                    {
                        strsql = "INSERT INTO ProjetoFilial VALUES (" + Sistran.Library.GetDataTables.RetornarIdTabela("PROJETOFILIAL") + ", " + ID + "," + lstEntrega.Items[i].Value + ",'ENTREGA')";
                        Sistran.Library.GetDataTables.ExecutarSemRetorno(strsql, "");
                    }
                }
