...
namespace SistranDAO
{
    public sealed class Produto
    {
        ...
        public DataTable ListarProdutoByDivisaoCliente(int Iddivisao, int idCliente, bool MaiorqueZero)
        {
            ...
            txt.Append(" LEFT JOIN UnidadeDeMedida UniDeMed  ");
            txt.Append(" ON (UniDeMed.IDUnidadeDeMedida = ProCli.IDUnidadeDeMedida)  ");
            txt.Append(" WHERE (EstDiv.IDClienteDivisao = " + Iddivisao.ToString() + " OR Est.SALDO>0  )   ");
            txt.Append(" AND ProCli.idcliente = " + idCliente.ToString());
            txt.Append(" AND COALESCE(Est.Saldo, 0) - (Select coalesce(Sum(DocIt.QuantidadeUnidadeEstoque),0) As QuantidadeUnidadeEstoque   ");
            txt.Append(" From DocumentoItem DocIt   ");
            txt.Append(" inner Join Documento Doc   ");
            txt.Append(" On (Doc.IdDocumento = DocIt.IdDocumento)   ");
            txt.Append(" inner Join ProdutoEmbalagem ProdE   ");
        ...
        public decimal RetornarSaldoDisponivelPorIdProdutoClienteDivisao(int idclienteDivisao, int idProdutoCliente, int idUnidadeDeArmazenagemLote)
        {
            System.Text.StringBuilder txt = new StringBuilder();
            ...
            //txt.Append(" SELECT DISTINCT    ");
            //txt.Append(" CAST(COALESCE(EST.SALDO, 0) - (SELECT COALESCE(SUM(DOCIT.QUANTIDADEUNIDADEESTOQUE),0) AS QUANTIDADEUNIDADEESTOQUE  ");
            //txt.Append(" FROM DOCUMENTOITEM DOCIT       ");
            //txt.Append(" INNER JOIN DOCUMENTO DOC ON (DOC.IDDOCUMENTO = DOCIT.IDDOCUMENTO)       ");
            //txt.Append(" INNER JOIN PRODUTOEMBALAGEM PRODE 	ON (PRODE.IDPRODUTOEMBALAGEM = DOCIT.IDPRODUTOEMBALAGEM)      ");
            //txt.Append(" WHERE DOC.ENTRADASAIDA = 'SAIDA' AND DOC.TIPODEDOCUMENTO = 'PEDIDO' AND DOC.ATIVO = 'SIM'   ");
            //txt.Append(" AND PRODE.IDPRODUTOCLIENTE = PROCLI.IDPRODUTOCLIENTE  AND COALESCE(DOCIT.ESTOQUEPROCESSADO,'NAO') = 'NAO'   ) AS NUMERIC (10,0)) AS SALDODIVISAODISPONIVEL   ");

            //txt.Append(" FROM  ESTOQUE EST      ");
            //txt.Append(" LEFT JOIN PRODUTOCLIENTE PROCLI   ON(PROCLI.IDPRODUTOCLIENTE = EST.IDPRODUTOCLIENTE)   ");
            //txt.Append(" LEFT JOIN PRODUTOEMBALAGEM PROEMB   ON (PROEMB.IDPRODUTOCLIENTE = PROCLI.IDPRODUTOCLIENTE) ");
            //txt.Append(" LEFT JOIN UNIDADEDEMEDIDA UNIDEMED   ON (UNIDEMED.IDUNIDADEDEMEDIDA = PROCLI.IDUNIDADEDEMEDIDA)    ");
            //txt.Append(" WHERE   PROCLI.IDPRODUTOCLIENTE= " + idProdutoCliente);


            txt.Append(" SELECT");
            txt.Append(" UAL.Saldo");
            txt.Append(" FROM  ESTOQUE EST       ");
            txt.Append(" LEFT JOIN PRODUTOCLIENTE PROCLI   ON(PROCLI.IDPRODUTOCLIENTE = EST.IDPRODUTOCLIENTE)    ");
            txt.Append(" LEFT JOIN PRODUTOEMBALAGEM PROEMB   ON (PROEMB.IDPRODUTOCLIENTE = PROCLI.IDPRODUTOCLIENTE) ");
            txt.Append(" LEFT JOIN UNIDADEDEMEDIDA UNIDEMED   ON (UNIDEMED.IDUNIDADEDEMEDIDA = PROCLI.IDUNIDADEDEMEDIDA)     ");
            txt.Append(" left join lote lt on lt.IDEstoque = EST.IDEstoque");
            txt.Append(" left join UnidadeDeArmazenagemLote ual on ual.IDLote = lt.IDLote");
            txt.Append(" WHERE   PROCLI.IDPRODUTOCLIENTE= " + idProdutoCliente + " and ual.IDUnidadeDeArmazenagemLote = " + idUnidadeDeArmazenagemLote);
            return Sistran.Library.GetDataTables.RetornarDecimal(txt.ToString(), HttpContext.Current.Session["Conn"].ToString());
        ...
        public DataTable Pesquisar(string texto, int idCliente)
        {
            ...
            //            txt.Append(" AND ProCli.idcliente =" + idCliente);
            //            txt.Append(" AND COALESCE(Est.Saldo, 0) - (Select coalesce(Sum(DocIt.QuantidadeUnidadeEstoque),0) As QuantidadeUnidadeEstoque    From DocumentoItem DocIt     ");
            ...
            txt.Append(" SELECT DISTINCT '' 'Nome', 0 'IDCLIENTEDIVISAO', '-' DIVISAO, ProCli.IDProdutoCliente, ProEmb.IDProdutoEmbalagem, ProEmb.IDProduto,   CASE WHEN ProCli.CodigoDoCliente is NULL THEN ProCli.Codigo ELSE 	ProCli.CodigoDoCliente   END as Codigo, UPPER(ProCli.Descricao) Descricao, ProEmb.Conteudo,     ");
            txt.Append(" Cast(COALESCE(Est.Saldo, 0) - (Select Coalesce(Sum(DocIt.QuantidadeUnidadeEstoque),0) As QuantidadeUnidadeEstoque ");
            txt.Append(" From DocumentoItem DocIt     inner Join Documento Doc On (Doc.IdDocumento = DocIt.IdDocumento)    ");
            txt.Append(" inner Join ProdutoEmbalagem ProdE 	on (ProdE.IdProdutoEmbalagem = DocIt.IdProdutoEmbalagem)      ");
            txt.Append(" where Doc.EntradaSaida = 'SAIDA'    and Doc.TipoDeDocumento = 'PEDIDO'     and Doc.Ativo = 'SIM'    and ProdE.IdProdutoCliente = ProCli.IDProdutoCliente  and Coalesce(DocIt.EstoqueProcessado,'NAO') = 'NAO'        ) as numeric (10,0)) AS SaldoDivisaoDisponivel,     ");
            txt.Append(" Cast(COALESCE(ProEmb.ValorUnitario, 0) as numeric (10,2)) As ValorUnitario,   UniDeMed.Decimais,     ");
            txt.Append(" 'R$ ' + Cast(Cast(COALESCE(ProEmb.ValorUnitario, 0) as numeric (10,2)) as varchar(12)) As RealValorUnitario,   null As nulo,     ");

            txt.Append(" lt.IDLote,  ");
            txt.Append(" convert(int, ual.Saldo) saldo, ");
            txt.Append(" lt.Validade, ");
            txt.Append(" ual.IDUnidadeDeArmazenagemLote, ");
            txt.Append(" ua.IDUnidadeDeArmazenagem, ");
            txt.Append(" dpl.Descricao endereco, lt.Referencia loteReferencia");
            ...
            txt.Append(" left join lote lt on lt.IDEstoque = EST.IDEstoque ");
            txt.Append(" left join UnidadeDeArmazenagemLote ual on ual.IDLote = lt.IDLote ");
            txt.Append(" left join UnidadeDeArmazenagem ua on ua.IDUnidadeDeArmazenagem = ual.IDUnidadeDeArmazenagem ");
            txt.Append(" left join DepositoPlantaLocalizacao dpl on dpl.IDDepositoPlantaLocalizacao = ua.IDDepositoPlantaLocalizacao ");

            txt.Append(" WHERE (  ProCli.Codigo like '%" + texto + "%' or    ProCli.Descricao like '%" + texto + "%')  ");
            txt.Append(" AND ProCli.idcliente =" + idCliente);
            txt.Append(" and  ual.saldo>0");
            txt.Append(" AND COALESCE(Est.Saldo, 0) - (Select coalesce(Sum(DocIt.QuantidadeUnidadeEstoque),0) As QuantidadeUnidadeEstoque    ");
            txt.Append(" From DocumentoItem DocIt      ");
            txt.Append(" inner Join Documento Doc    On (Doc.IdDocumento = DocIt.IdDocumento)      ");
            txt.Append(" inner Join ProdutoEmbalagem ProdE    on (ProdE.IdProdutoEmbalagem = DocIt.IdProdutoEmbalagem)     ");
            ...
            txt.Append(" order by CASE WHEN ProCli.CodigoDoCliente is NULL THEN ProCli.Codigo ELSE 	ProCli.CodigoDoCliente   END, lt.IDLote, lt.Referencia");
        ...
        {
            System.Text.StringBuilder txt = new StringBuilder();
            txt.Append("  SELECT PRO.IDPRODUTO, PRO.CODIGODEBARRAS,ISNULL( PRO.PESOLIQUIDO,0) PESOLIQUIDO, ISNULL(PRO.PESOBRUTO, 0) PESOBRUTO, ISNULL(PRO.ESPECIE, 0) ESPECIE, ");
            txt.Append(" ISNULL(PRO.LARGURA, 0) LARGURA ,  ISNULL(PRO.ALTURA, 0) ALTURA , ISNULL( PRO.COMPRIMENTO,0) COMPRIMENTO,");
