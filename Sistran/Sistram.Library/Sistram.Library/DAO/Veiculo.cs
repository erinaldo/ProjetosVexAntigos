...
namespace SistranDAO
{
    public class Veiculo
    {
        public DataTable ListarMonitoramento(string data, string filiais, string cliente, string ordem, string Conn)
        {
            ...
            strsql.Append(" Select    rast.UltimaSincronizacao, fil.Nome filial, Dt.Numero,   Dt.Numero NDt,   Dt.Emissao,   Vei.Placa, Dt.IDDT, ");
            ...
            strsql.Append(" (select count(idrastreamento) from rastreamento rr where rr.iddt = Dt.IDDT) idrastreamento ");
            strsql.Append(" From Dt Dt    WITH (NOLOCK) ");
            ...
            strsql.Append(" left join rastreador rast on rast.ultimadt = dt.numero ");
            strsql.Append(" where  @filial  and dt.ativo='SIM'");
            ...
            strsql.Append(" Group By    rast.UltimaSincronizacao,fil.Nome, Dt.Numero,   Dt.IdDt,   Dt.Emissao,   Vei.Placa, Dt.IDDT, Cad.RazaoSocialNome  ");
        ...
        public DataTable ListarMonitoramentoInterno(string data, string filiais, string cliente, string ordem, string Conn)
        {
            StringBuilder strsql = new StringBuilder();
            strsql.Append(" Select    UltimaSincronizacao, UltimaDT,UltimaPlaca,InicioSincronizacao,FinalSincronizacao,TempoSincronizacao,UtltimoEnvioDeDados, fil.Nome filial, Dt.Numero,   Dt.Numero NDt,   Dt.Emissao,   Vei.Placa, Dt.IDDT, ");
            strsql.Append("Coalesce(Count(Doc.Numero),0) As Documentos, ");
            strsql.Append("Coalesce(Count(Distinct Doc.Endereco),0) As TotalDeServicos,   ");
            strsql.Append("Coalesce(Count(Case when Doc.IdDocumentoOcorrencia is Not Null then Doc.Numero else Null end),0) As Ocorrencias,  ");
            strsql.Append("Coalesce(Count(Case when Doc.DataDeConclusao is Not Null then Doc.Numero else Null end),0) As DocumentosConcluido,   ");
            ...
            strsql.Append("Coalesce(Sum(Doc.Volumes),0) as Volumes, Cad.RazaoSocialNome as Motorista");
            strsql.Append(" From Dt Dt    WITH (NOLOCK) ");
            strsql.Append(" INNER Join DTRomaneio DTRom on (DtRom.IdDt=Dt.IdDt)    ");
            strsql.Append(" INNER Join Romaneio Rom on (Rom.IdRomaneio=DtRom.IdRomaneio)    ");
            strsql.Append(" INNER Join RomaneioDocumento RomDoc on (RomDoc.IdRomaneio=Rom.IdRomaneio)    ");
            strsql.Append(" INNER Join Documento Doc on (Doc.IdDocumento=RomDoc.IdDocumento)    ");
            strsql.Append(" INNER Join Veiculo Vei on (Vei.IdVeiculo=Dt.IdPrimeiroVeiculo)  ");
            strsql.Append(" INNER join Cadastro Cad on Vei.IDMotorista = Cad.IDCadastro");
            strsql.Append(" INNER join Filial fil on DT.IDFilial = fil.IDFilial");
            strsql.Append(" INNER join rastreador rast on rast.ultimadt = dt.numero ");
            strsql.Append(" where  @filial  and dt.ativo='SIM'");
            strsql.Append(" and  cast(DT.DATADESAIDA as date) = cast(CONVERT(datetime, '@data',103) as date) ");
            strsql.Append(" and Vei.Placa is not null ");
            strsql.Append(" and (Doc.IdRemetente in (@cliente)  or IDCliente in(@cliente)) ");
            strsql.Append(" Group By    UltimaSincronizacao, UltimaDT,UltimaPlaca,InicioSincronizacao,FinalSincronizacao,TempoSincronizacao,UtltimoEnvioDeDados, fil.Nome, Dt.Numero,   Dt.IdDt,   Dt.Emissao,   Vei.Placa, Dt.IDDT, Cad.RazaoSocialNome  ");
            strsql.Append(" Having (Coalesce(Count(Doc.Numero),0) >0)  ");
            strsql.Append("  @ordem ");
        ...
        {
            DataTable dtProp = new SistranDAO.Cadastro.Proprietario().Pesquisar("", "", IDPROPRIETARIO, null);
