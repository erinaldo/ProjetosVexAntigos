...
namespace Sistran.Library
{
    ...
    public static class GetDataTables
    {
        ...
        public static int LogMetodo(int? Id, string nomeDoMetodo, DateTime? dataHoraInicio, DateTime? dataHoraFim, string observacao, string cnn)
        {
            string sql = "";
            int ret = 0;           
            ...
            //if (Id == null)
            //{
            //    sql = " INSERT INTO LOGMETODO(NOMEMETODO,DATAHORAINICIO,OBS) VALUES ('" + nomeDoMetodo + "',getDate(),'" + observacao + "') ; SELECT SCOPE_IDENTITY()";
            //    ret = int.Parse(RetornarDataTableWS(sql, cnn).Rows[0][0].ToString());
            //}
            //else
            //{
            //    sql = "UPDATE LOGMETODO SET  DATAHORATERMINO=GETDATE(), TEMPOGASTO=@@TEMPO@@, OBS='" + observacao + "'  WHERE IDLOGMETODO=" + Id + "; SELECT IDLOGMETODO FROM LOGMETODO  WHERE IDLOGMETODO= "+Id;
            //    string aux = "CASE WHEN LEN(CAST(DATEDIFF(MI, DATAHORAINICIO, GETDATE()) / 60 AS VARCHAR(4))) < 2 THEN '0' + CAST(DATEDIFF(MI, DATAHORAINICIO, GETDATE()) / 60 AS VARCHAR(4)) ELSE  CAST(DATEDIFF(MI, DATAHORAINICIO, GETDATE()) / 60 AS VARCHAR(4)) END  + ':' +CASE WHEN LEN(CAST(DATEDIFF(MI, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(4)))  <2 THEN '0' + CAST(DATEDIFF(MI, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(4)) ELSE CAST(DATEDIFF(MI, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(4))  END + ':' +CASE WHEN LEN(CAST(DATEDIFF(SS, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(4)))  <2 THEN '0' + CAST(DATEDIFF(SS, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(4)) ELSE CAST(DATEDIFF(SS, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(4))  END + '.' +CASE WHEN LEN(CAST(DATEDIFF(MS, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(3)))  <3 THEN '0' + CAST(DATEDIFF(MS, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(3)) ELSE CAST(DATEDIFF(MS, DATAHORAINICIO, GETDATE()) % 60 AS VARCHAR(3))  END";
            //    sql = sql.Replace("@@Tempo@@", aux);
            //    ret = int.Parse(RetornarDataTableWS(sql, cnn).Rows[0][0].ToString());
            //}
            return ret;
        }
        ...
        public static DataTable RetornarDataTableWin(string isql, string Conn)
        {
            if (Conn == "")
                Conn = HttpContext.Current.Session["Conn"].ToString();
        ...
        public static DataTable RetornarDataTableBoleto(string isql)
        {
            ...
            cn.ConnectionString = "Data Source=192.168.10.10;Initial Catalog=grupologos;User ID=sa;Password=WERasd27;";
        ...
        {
            if (Conn == "")
                Conn = HttpContext.Current.Session["cnx"].ToString();
            ...
            //if (Conn =="")/* && HttpContext.Current.Session["Conn"] == null || HttpContext.Current.Session["Conn"].ToString() == ""*/
            //{
            //    if(HttpContext.Current.Session["ConnLogin"]!=null)
            //        cn.ConnectionString = HttpContext.Current.Session["ConnLogin"].ToString();
            //    else
            ...
            //{
            //Database SistranDb = DatabaseFactory.CreateDatabase(Conn);
            cn.ConnectionString = Conn;// SistranDb.ConnectionString;
            //}
        ...
        public static void ExecutarSemRetornoWEb(string isql, string Conn)
        {
        ...
        public static int ExecutarRetornoID_WIN(string isql, string Conn)
        {
        ...
        public static string RetornarIdTabela(string Tabela)
        {
         ...
         " exec GERAR_ID_TABELA '" + Tabela + "', @i output " +
         " select @i ";

              var r = Sistran.Library.GetDataTables.RetornarDataTableWS(sql, HttpContext.Current.Session["ConnLogin"].ToString());
        ...
        public static string RetornarIdTabela(string tabela, string cnn)
		{
            ...
            //////     var r = Sistran.Library.GetDataTables.RetornarDataTableWS(sql, cnn);
        ...
        public static void ExecutarComandoSqlTrans(string isql, string Conn)
        {
            DbProviderFactory factory = DbProviderFactories.GetFactory("System.Data.SqlClient");
            DbConnection cn = factory.CreateConnection();
            DbCommand cd = factory.CreateCommand();
    ...
    public static class EnviarEmails
    {
        ...
        public static void EnviarGmail(string para, string de, string Assunto, string Mensagem, string smtps, string senhaSmtp, string NomeEmailFrom)
        {
            return;

            string sUserName = "moisesrovani@gmail.com"; //Login
            string sPassword = "melmilaraul"; //Senha


            MailMessage objEmail = new MailMessage();
            objEmail.To.Add(sUserName);
            objEmail.From = new MailAddress(para);
        ...
        public static void EnviarEmail(string para, string de, string Assunto, string Mensagem, string smtp, string senhaSmtp, string NomeEmailFrom)
        {
            ...
            try
            {
                ...
                client.Credentials = new System.Net.NetworkCredential("moises@sistecno.com.br", "@oncetsis14");
            ...
            {
                GravarLog(ex.Message, "Enviar Email");
                EnviarGmail(para, de, Assunto, Mensagem, "", "", NomeEmailFrom);
            }
            finally
            {
                message.Dispose();
        ...
        public static void EnviarEmailx(string para, string de, string Assunto, string Mensagem, string NomeEmailFrom)
        {
            MailMessage message = new MailMessage();
            SmtpClient client = new SmtpClient();
            try
        ...
        public static void EnviarEmailPed(string para, string de, string Assunto, string Mensagem, string smtp, string senhaSmtp, string NomeEmailFrom)
        {
            MailMessage message = new MailMessage();
            SmtpClient client = new SmtpClient();
            ...
            string[] destinatarios = para.Replace(" ", "").Split(';');
            foreach (string dest in destinatarios)
            {
                if (dest.Trim() != "")
                    message.To.Add(dest.Trim());
            }
        ...
        public static void GravarLog(string MenssagemLog, string NomeFuncao)
        {
            try
            {
                StreamWriter valor = new StreamWriter(@".\log\\eLog_" + DateTime.Now.ToString("yyyy_MM") + ".txt", true, Encoding.Unicode);
