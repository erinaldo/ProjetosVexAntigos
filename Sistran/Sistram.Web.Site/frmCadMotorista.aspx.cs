...
public partial class Intranet_frmCadMotorista : System.Web.UI.Page
{
    ...
    private void preencherCampos()
    {
       ...
       // Session["lblId"] = lblId.Text;
        ...
        txtPamcary.Text = (dt.Rows[0]["VencimentoPamcary"].ToString() == "" ? "" : Convert.ToDateTime(dt.Rows[0]["VencimentoPamcary"]).ToShortDateString());
    ...
    private void CarregarListaFilial()
    {
        if (Request.QueryString["IDMOTORISTA"] != null)
        {
            ...
            lstTodasFiliais.DataSource = new SistranBLL.Filial().ListarDisponiveisByIDMotorista("", int.Parse(Request.QueryString["IDMOTORISTA"].ToString()),false);            
        }
        else
        {
            lstTodasFiliais.DataSource = new SistranBLL.Filial().ListarDisponiveisByIDMotorista("", 0, false);
        }
