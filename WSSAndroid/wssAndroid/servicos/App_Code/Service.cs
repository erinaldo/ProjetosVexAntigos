...
/*
 ...
 Create Procedure PRC_INCLUIR_CHAVE (@CHAVE VARCHAR(44), @PLACA VARCHAR(10), @IDTIPOOPERACAO INT)
AS
BEGIN
	DECLARE @EXISTE_CHAVE VARCHAR(44)
	
	SELECT @EXISTE_CHAVE = Chave FROM Chave WHERE CHAVE =@CHAVE

	IF(ISNULL(@EXISTE_CHAVE, '') <> '')
	BEGIN
		UPDATE Chave SET Placa = @PLACA, IDDT=NULL, IdTipoDeOperacao = @IDTIPOOPERACAO
		WHERE Chave = @CHAVE
	END 
	ELSE
	BEGIN
		Insert into Chave (Chave, DataDeCadastro, Placa, IdTipoDeOperacao)  values (@CHAVE, getdate(), @PLACA, @IDTIPOOPERACAO)
	END
END
--------------------------------------------------------------------------------------------------------------
 * 
 CREATE PROCEDURE PRC_NOTAS_LIBERADAS_DT (@PLACA VARCHAR(10))
AS
SELECT   C.CHAVE, 
CAST(SUBSTRING(C.CHAVE, 26,9) AS INT) NFE,
TP.NOME OPERACAO,
D.VALORDANOTA,
DEST.FANTASIAAPELIDO,
CASE ISNULL(IMPORTADO, '') WHEN '' THEN 'P' ELSE 'I' END STATUS

FROM CHAVE C 
LEFT JOIN DOCUMENTO D ON D.DOCUMENTODOCLIENTE4 = C.CHAVE
LEFT JOIN TIPODEOPERACAO TP ON TP.IDTIPODEOPERACAO = C.IDTIPODEOPERACAO
LEFT JOIN CADASTRO DEST ON DEST.IDCADASTRO = D.IDDESTINATARIO
WHERE PLACA = @PLACA
AND IDDT IS NULL
AND C.IDTIPODEOPERACAO IS NOT NULL
ORDER BY 2, 1
...
public class Service : System.Web.Services.WebService
{
        ...
        string senhaSeguranca)
    {
        ...
        try
        {
            ...
            strsql += " IDDocumentoOcorrencia, IdUsuario,";
            strsql += " IDDocumento,";
            strsql += " IDFilial,";
            strsql += " IDOcorrencia,";
            ...
            strsql += " ) VALUES (";
            strsql += IdDocOco + " ,";
            strsql += "(Select IdUsuario from Usuario where Login='RSMOBILE'),";
            strsql += idDocumento + " ,";
            // strsql += Convert.ToInt32(IDFilial) + " ,";
            strsql += "isnull((Select IdFilialAtual from Documento with (nolock) where IdDocumento=" + idDocumento + "), 1)" + " ,";
            ...
            strsql += latitude + ", ";
            strsql += longitude + ", ";
            strsql += "(select top 1 re.IdRomaneio  From RomaneioDocumento  re Inner join Romaneio r on r.IdRomaneio = re.IdRomaneio where IdDocumento = " + idDocumento + " and r.Tipo = 'Entrega' Order by 1 desc)";
            strsql += " ) select '1' ";
            ...
            strsql = "UPDATE DOCUMENTO SET IDDOCUMENTOOCORRENCIA = " + IdDocOco + " " + (finalizadora == "SIM" ? " , DATADECONCLUSAO= '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'" : "") + "  WHERE IDDocumento=" + idDocumento + " Select '1'";
            ...
            if (finalizadora == "SIM")
            {
                cd.CommandText = "Update DocumentoFilial set Situacao='PROCESSO FINALIZADO', data=getdate() where IdDocumento=" + idDocumento;
            }
            else
            {
                cd.CommandText = "Update DocumentoFilial set Situacao='AGURDANDO EMBARQUE', data=getdate() where IdDocumento=" + idDocumento;
            ...
            if (image != null)
            {
                ...
                byte[] readBytes = Convert.FromBase64String(image);

                command.Parameters.Add(new SqlParameter("@IMAGEM", readBytes));
                vv.Open();
    ...
    public void Limpar(string PLACA, string DOCTRANSP, string cd_cliente, string senhaSeguranca)
    {
        ...
        strsql += "  WHERE REPLACE(VEI.PLACA, '-','')  = '" + PLACA.Replace("-","") + "' AND TIPODEDOCUMENTO='NOTA FISCAL' AND DOC.ATIVO='SIM' AND DT.NUMERO ='" + DOCTRANSP + "'  ";
    ...
    public listar_documentos[] Listar_Documentos(string PLACA, string DOCTRANSP, string EQUIPAMENTO, string cd_cliente, string senhaSeguranca)
    {
        ...
        try
        {
            ...
            SQL += " WHERE REPLACE(VEI.PLACA, '-','')  = '" + PLACA.Replace("-", "") + "'  AND DOC.ATIVO='SIM'";
            ...
            //dt.WriteXml(@"c:\teste\Documentos.xml");
             ...
             //Sistran.Library.EnviarEmails.EnviarEmail("moises@sistecno.com.br", "moises@sistecno.com.br", "Teste", SQL + "-Placa:" + PLACA + "-" + RetornarStringConexao(cd_cliente, senhaSeguranca), "mail.sistecno.com.br", "mo2404");
    ...
    [WebMethod]
    public TipoDeOperacao[] RetornarTipoDeOperacao(string cd_cliente, string senhaSeguranca)
    {
        try
        {
            string SQL = "SELECT IDTIPODEOPERACAO, NOME FROM TIPODEOPERACAO WHERE ATIVO = 'SIM'";

            List<TipoDeOperacao> Lista = new List<TipoDeOperacao>();
            ...
            foreach (DataRow item in dt.Rows)
            {
                TipoDeOperacao itemResult = new TipoDeOperacao();
                itemResult.idTipoDeOperacao = item["idTipoDeOperacao"].ToString();
                itemResult.Nome = item["Nome"].ToString();
                Lista.Add(itemResult);
            }
    ...
    [WebMethod]
    public bool ValidarPlaca(string cd_cliente, string senhaSeguranca, string placa)
    {
        try
        {
            string SQL = "select Count(*) from Veiculo where replace(Placa, '-','') = replace('"+placa+"', '-','') ";
            List<TipoDeOperacao> Lista = new List<TipoDeOperacao>();
            DataTable dt = ExecutaBD.RetornarDataTable(SQL, RetornarStringConexao(cd_cliente, senhaSeguranca));
    ...
    [WebMethod]
    public DataTable DtsEmAberto(string cd_cliente, string senhaSeguranca, string placa)
    {
        try
        {
            string SQL = "SELECT DT.IDDT, DT.NUMERO, CONVERT(VARCHAR(10), DT.EMISSAO, 103) EMISSAO,  ";
            SQL += " ( ";
            SQL += " 	SELECT TOP 1 NOME  ";
            SQL += " 	FROM DOCUMENTO D ";
            SQL += " 	INNER JOIN TIPODEOPERACAO TOO ON TOO.IDTIPODEOPERACAO = D.IDTIPODEOPERACAO  ";
            SQL += " 	INNER JOIN DTROMANEIO DTR ON DTR.IDDT =  DT.IDDT ";
            SQL += " 	WHERE DT.IDDT = DTR.IDDT ";
            SQL += " ) OPERACAO  , ";
            SQL += " ( ";
            SQL += " 	SELECT COUNT(DISTINCT D.IDDOCUMENTO) ";
            SQL += " 	FROM DOCUMENTO D		 ";
            SQL += " 	INNER JOIN ROMANEIODOCUMENTO RD ON RD.IDDOCUMENTO = D.IDDOCUMENTO ";
            SQL += " 	INNER JOIN DTROMANEIO DTR ON DTR.IDROMANEIO = RD.IDROMANEIO ";
            SQL += " WHERE DTR.IDDT = DT.IDDT ";
            SQL += " ) DOCUMENTOS   ";

            SQL += " FROM DT  ";
            SQL += " INNER JOIN VEICULO V ON V.IDVEICULO = DT.IDPRIMEIROVEICULO ";
            SQL += " WHERE  ";
            SQL += " DT.SITUACAO='ABERTO'  ";
            SQL += " AND ANDAMENTO='EM ENTREGA'   ";
            SQL += " AND DT.ATIVO='SIM' ";
            SQL += " AND  REPLACE(PLACA, '-','') = REPLACE('" + placa + "', '-','') ";
            SQL += " ORDER BY 3 DESC ";
            List<TipoDeOperacao> Lista = new List<TipoDeOperacao>();
            DataTable dt = ExecutaBD.RetornarDataTable(SQL, RetornarStringConexao(cd_cliente, senhaSeguranca));
    ...
    [WebMethod]
    public void LiberarGeracaoDT(string cd_cliente, string senhaSeguranca, string placa)
    {
        ...
        try
        {
            string strsql = "Update Chave Set Protocolo='"+ DateTime.Now.ToString("yyyyMMddHHmmssffftt") +"' where Placa='"+placa+"'";
            cd.CommandText = strsql;
            cd.CommandType = CommandType.Text;
            cd.Transaction = oTrans;
            cd.ExecuteScalar();
    ...
    [WebMethod]
    public void GravarChaveGerarDt(string cd_cliente, string senhaSeguranca, string chave, string placa, string IdTipoDeOperacao, string protocolo)
    {
        ...
        try
        {
            string strsql = "Insert into Chave (Chave, DataDeCadastro, Placa, IdTipoDeOperacao, Protocolo)  values ('" + chave + "', getdate(), '" + placa + "', " + IdTipoDeOperacao + ", '"+protocolo+"')";
            cd.CommandText = strsql;
            cd.CommandType = CommandType.Text;
            cd.Transaction = oTrans;
            cd.ExecuteScalar();
    ...
    [WebMethod]
    public void GravarChave(string cd_cliente, string senhaSeguranca, string chave, string placa, string IdTipoDeOperacao)
    {
        ...
        try
        {
            ...
            //string strsql = "Insert into Chave (Chave, DataDeCadastro, Placa, IdTipoDeOperacao)  values ('" + chave + "', getdate(), '" + placa + "', " + IdTipoDeOperacao + ")";
            string strsql = " EXEC PRC_INCLUIR_CHAVE '" + chave + "', '" + placa + "', " + IdTipoDeOperacao ;            
            cd.CommandText = strsql;
            cd.CommandType = CommandType.Text;
            cd.Transaction = oTrans;
            cd.ExecuteScalar();
    ...
    [WebMethod]
    public DataTable RetornarNotasPelaPlaca(string cd_cliente, string senhaSeguranca, string placa)
    {

        try
        {

            string strsql = " EXEC Prc_Notas_Liberadas_DT  '" + placa + "'";
            return ExecutaBD.RetornarDataTable(strsql, RetornarStringConexao(cd_cliente, senhaSeguranca));
    ...
    [WebMethod]
    public void RetirarNota(string cd_cliente, string senhaSeguranca, string placa, string chave)
    {
        try
        {

            string strsql = " Update Chave set Placa='AAA0000' where Placa='"+placa+"' and Chave='"+chave+"'; Select 1 ";
            ExecutaBD.RetornarDataTable(strsql, RetornarStringConexao(cd_cliente, senhaSeguranca));
...
//            strsql += " convert(datetime,'" + DateTime.Parse(DataHoraOcorrencia).ToString("dd/MM/yyyy HH:mm:ss") + "', 103) ,";
            ...
            strsql = "UPDATE DOCUMENTO SET IDDOCUMENTOOCORRENCIA = " + IdDocOco + " " + (finalizadora == "SIM" ? " , DATADECONCLUSAO= convert(datetime,'" + DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + "', 103)" : "") + "  WHERE IDDocumento=" + idDocumento + " Select '1'";
            //--            ExecutaBD.ExecutarSQLRetornarIDs(strsql, RetornarStringConexao(cd_cliente));
            ...
            {
                cd.CommandText = "Update DocumentoFilial set Situacao='AGUARDANDO EMBARQUE', data=getdate() where IdDocumento=" + idDocumento;
        ...
        string senhaSeguranca)
    {
        ...
        try
        {
            ...
            strsql += int.Parse(idOcorrencia) + " ,";
            //strsql += " getDate() ,";
            strsql += "'" + DateTime.Parse(dataHoraOcorencia).ToString("dd/MM/yyyy HH:mm:ss") + "' ,";
            strsql += " '" + descricaoOcorrencia.ToUpper().Trim() + "',";
            strsql += "'SIM', ";
            strsql += latitude + ", ";
            strsql += longitude;
    ...
    {
        public string idTipoDeOperacao { get; set; }
        public string Nome { get; set; }
    ...
    public static void enviarEmailGrupoLogos(string assunto, string corpo)
    {
        ...
        //System.Net.NetworkCredential credenciais = new System.Net.NetworkCredential("moises@sistecno.com.br", "@oncetsis14");
