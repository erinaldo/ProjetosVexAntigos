				...
				if(dtSincronizar!=null)
				{
					...
					while (itr.hasNext()) 
					{
						...
						if(buffer !=null &&	buffer.length>0)
						{
							...
							{
								gravou = call.GravarOcorrencias(element.get_IDDOCUMENTO(),element.get_IDOCORRENCIA(),"EFETUADO PELO ANDROID","1", buffer, this,element.get_LONGITUDE(),element.get_LATTUDE(),	element.get_IDDT(),	element.get_DATAHORAOCORRENCIA(),element.get_DATAHORAOCORRENCIA(), cd_cliente);
						...
						else
						{
							if(cd_cliente.toString()=="2") // teste
								...
								gravou = call.GravarOcorrenciasSemImagem(element.get_IDDOCUMENTO(), element.get_IDOCORRENCIA(), "EFETUADO PELO ANDROID", "1", this,	element.get_LONGITUDE(),element.get_LATTUDE(),	element.get_IDDT(),	element.get_DATAHORAOCORRENCIA(),element.get_DATAHORAOCORRENCIA(), cd_cliente);
