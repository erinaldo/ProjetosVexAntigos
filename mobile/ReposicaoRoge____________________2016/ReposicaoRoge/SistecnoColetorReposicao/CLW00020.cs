...
{
    public partial class CLW00020 : Form
    {
        public CLW00020()
        {
            InitializeComponent();
            ...
            try
            {
                if (txtUa.Text != "" && txtUaDestino.Text != "")
                {
                   /* new Classes.BLL.Estoque().SairUAInteira(dtUa.Rows[0]["IdUnidadeDeArmazenagem"].ToString(), lblIdProdutoCliente.Text, lblSaldoUa.Text, txtEndereco.Text, txtUaDestino.Text, dtUa.Rows[0]["IDPRODUTOEMBALAGEM"].ToString());*/
                    ...
                    DataTable dtrom = Classes.BdExterno.RetornarDT("select * from romaneio where idromaneio=" + idRom[0], VarGlobal.Conexao);

                    DataTable DtConfPallet = Classes.BdExterno.RetornarDT("Select * from ConferenciaPallet where idConferencia=" + idRom[1] + " and IdPallet=" + dtUa.Rows[0]["IdUnidadeDeArmazenagem"].ToString(), VarGlobal.Conexao);

                    if (DtConfPallet.Rows.Count > 0)
                    {
                        MessageBox.Show("Pallet já incluido no Romaneio: " +idRom[0] );
                        InciaControles();
                        return;
                    }
                    ...
                    {
                        MessageBox.Show("Romaneio: "+ idRom[0] +" não encontrado.");
                        InciaControles();
                        return;
                    }
                   ...
                   string  idConferenciaPallet = BdExterno.RetornarIDTabela("CONFERENCIAPALLET").ToString();
                    sql += "INSERT INTO CONFERENCIAPALLET (IdConferenciaPallet, IdConferencia, IdPallet, IdDepositoPlantaLocalizacao, TIPO, UACompleta)";
                    sql += " VALUES (" + idConferenciaPallet + ", " + idRom[1] + ", " + dtUa.Rows[0]["IdUnidadeDeArmazenagem"].ToString() + ", " + dtrom.Rows[0]["IDDEPOSITOPLANTALOCALIZACAO"].ToString() + ", 'CONFERENCIA', 'SIM')";
