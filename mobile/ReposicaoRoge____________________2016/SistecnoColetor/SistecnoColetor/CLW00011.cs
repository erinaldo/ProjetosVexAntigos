        ...
        private void txtUa_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                ...
                if (e.KeyChar == (char)Keys.Return && txtUa.Text != "")
                {
                    ...
                    string sql = "exec PRC_VerificaSaldoDaUa " + Helpers.RetirarDigitoUA(txtUa.Text);

                    if (Classes.BdExterno.RetornarDT(sql, VarGlobal.Conexao).Rows.Count == 0)
                    {
                        txtUa.SelectAll();
                        throw new Exception("UA VAZIA. Verifique o saldo e a armazenagem da UA: " + Helpers.RetirarDigitoUA(txtUa.Text));
                    }
